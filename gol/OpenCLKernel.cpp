...
*/
OpenCLKernel::OpenCLKernel(int platformId, int deviceId, int nbWorkingItems,
                           int draft)
    : m_hContext(0), m_hQueue(0), m_hBitmap(0), m_hBuffer(0), m_hVideo(0),
      m_hDepth(0),
#if USE_KINECT
      m_skeletons(0), m_hNextDepthFrameEvent(0), m_hNextVideoFrameEvent(0),
      m_hNextSkeletonEvent(0), m_pVideoStreamHandle(0), m_pDepthStreamHandle(0),
      m_skeletonsBody(-1), m_skeletonsLamp(-1),
#endif // USE_KINECT
      m_hTextures(0), m_textures(0), m_texturedTransfered(false), m_offset(-1),
      m_timer(0.f) {
  int status(0);
...
#if USE_KINECT
  // Initialize Kinect
  status = NuiInitialize(NUI_INITIALIZE_FLAG_USES_DEPTH_AND_PLAYER_INDEX |
  ...
  status =
      NuiImageStreamOpen(NUI_IMAGE_TYPE_COLOR, NUI_IMAGE_RESOLUTION_640x480, 0,
  ...
  int p = platformId;
  {
    // Platform details
    s << "Platform " << p << ":\n";
    CHECKSTATUS(clGetPlatformInfo(platforms[p], CL_PLATFORM_PROFILE,
                                  MAX_SOURCE_SIZE, buffer, &len));
    buffer[len] = 0;
    s << "  Profile    : " << buffer << "\n";
    CHECKSTATUS(clGetPlatformInfo(platforms[p], CL_PLATFORM_VERSION,
                                  MAX_SOURCE_SIZE, buffer, &len));
    buffer[len] = 0;
    s << "  Version    : " << buffer << "\n";
    CHECKSTATUS(clGetPlatformInfo(platforms[p], CL_PLATFORM_NAME,
                                  MAX_SOURCE_SIZE, buffer, &len));
    buffer[len] = 0;
    s << "  Name       : " << buffer << "\n";
    CHECKSTATUS(clGetPlatformInfo(platforms[p], CL_PLATFORM_VENDOR,
                                  MAX_SOURCE_SIZE, buffer, &len));
    buffer[len] = 0;
    s << "  Vendor     : " << buffer << "\n";
    CHECKSTATUS(clGetPlatformInfo(platforms[p], CL_PLATFORM_VENDOR,
    ...
    CHECKSTATUS(clGetDeviceIDs(platforms[p], CL_DEVICE_TYPE_ALL, 1, m_hDevices,
                               &ret_num_devices));
    ...
    // for( cl_uint d(0); d<ret_num_devices; d++ )
    {
      ...
      CHECKSTATUS(clGetDeviceInfo(m_hDevices[d], CL_DEVICE_NAME, sizeof(buffer),
                                  buffer, NULL));
      s << "    DEVICE_NAME                        : " << buffer << "\n";
      CHECKSTATUS(clGetDeviceInfo(m_hDevices[d], CL_DEVICE_VENDOR,
                                  sizeof(buffer), buffer, NULL));
      s << "    DEVICE_VENDOR                      : " << buffer << "\n";
      CHECKSTATUS(clGetDeviceInfo(m_hDevices[d], CL_DEVICE_VERSION,
                                  sizeof(buffer), buffer, NULL));
      s << "    DEVICE_VERSION                     : " << buffer << "\n";
      CHECKSTATUS(clGetDeviceInfo(m_hDevices[d], CL_DRIVER_VERSION,
                                  sizeof(buffer), buffer, NULL));
      s << "    DRIVER_VERSION                     : " << buffer << "\n";

      cl_uint value;
      cl_uint values[10];
      CHECKSTATUS(clGetDeviceInfo(m_hDevices[d], CL_DEVICE_MAX_COMPUTE_UNITS,
      ...
      CHECKSTATUS(clGetDeviceInfo(m_hDevices[d], CL_DEVICE_MAX_WORK_GROUP_SIZE,
                                  sizeof(value), &value, NULL));
      s << "    CL_DEVICE_MAX_WORK_GROUP_SIZE      : " << value << "\n";
      CHECKSTATUS(clGetDeviceInfo(m_hDevices[d], CL_DEVICE_MAX_WORK_ITEM_SIZES,
                                  sizeof(values), &values, NULL));
      s << "    CL_DEVICE_MAX_WORK_ITEM_SIZES      : " << values[0] << ", "
        << values[1] << ", " << values[2] << "\n";
      CHECKSTATUS(clGetDeviceInfo(m_hDevices[d], CL_DEVICE_MAX_CLOCK_FREQUENCY,
                                  sizeof(value), &value, NULL));
      s << "    CL_DEVICE_MAX_CLOCK_FREQUENCY      : " << value << "\n";

      cl_device_type infoType;
      CHECKSTATUS(clGetDeviceInfo(m_hDevices[d], CL_DEVICE_TYPE,
                                  sizeof(infoType), &infoType, NULL));
      s << "    DEVICE_TYPE                        : ";
  ...
  m_hContext = clCreateContext(NULL, ret_num_devices, &m_hDevices[0], NULL,
                               NULL, &status);
  m_hQueue = clCreateCommandQueue(m_hContext, m_hDevices[0],
...
* compileKernels
*/
void OpenCLKernel::compileKernels(const KernelSourceType sourceType,
  ...
  try {
    ...
    hProgram =
        clCreateProgramWithSource(m_hContext, 1, (const char **)&source_str,
    ...
    // if( m_computeUnits == 0 )
    {
      clGetKernelWorkGroupInfo(m_hMainKernel, m_hDevices[0],
                               CL_KERNEL_WORK_GROUP_SIZE,
                               sizeof(m_computeUnits), &m_computeUnits, NULL);
      std::cout << "CL_KERNEL_WORK_GROUP_SIZE=" << m_computeUnits << std::endl;
    }

    clGetKernelWorkGroupInfo(m_hMainKernel, m_hDevices[0],
                             CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE,
                             sizeof(m_preferredWorkGroupSize),
                             &m_preferredWorkGroupSize, NULL);
    std::cout << "CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE="
              << m_preferredWorkGroupSize << std::endl;

    char buffer[MAX_SOURCE_SIZE];
    LOG_INFO("clGetProgramBuildInfo\n");
    CHECKSTATUS(
        clGetProgramBuildInfo(hProgram, m_hDevices[0], CL_PROGRAM_BUILD_LOG,
                              MAX_SOURCE_SIZE * sizeof(char), &buffer, &len));
    ...
    if (ptxFileName.length() != 0) {
      ...
      hProgram = clCreateProgramWithBinary(
          m_hContext, 1, &m_hDevices[0], &lSize,
...
void OpenCLKernel::initializeDevice(int width, int height) {
  ...
  m_hBitmap =
      clCreateBuffer(m_hContext, CL_MEM_WRITE_ONLY,
                     width * height * sizeof(BYTE) * gColorDepth, 0, NULL);
  m_hBuffer = clCreateBuffer(m_hContext, CL_MEM_READ_WRITE,
                             2 * width * height * sizeof(cl_float4), 0, NULL);
  m_hTextures = clCreateBuffer(
      m_hContext, CL_MEM_READ_ONLY,
      gTextureWidth * gTextureHeight * gTextureDepth * sizeof(BYTE), 0, NULL);
  m_hVideo =
      clCreateBuffer(m_hContext, CL_MEM_READ_ONLY,
                     gVideoWidth * gVideoHeight * gKinectColorVideo, 0, NULL);
  m_hDepth =
      clCreateBuffer(m_hContext, CL_MEM_READ_ONLY,
                     gDepthWidth * gDepthHeight * gKinectColorDepth, 0, NULL);
...
* runKernel
*/
void OpenCLKernel::render(int width, int height, BYTE *bitmap, float value) {
  int status(0);
  ...
  if (!m_texturedTransfered) {
    CHECKSTATUS(
        clEnqueueWriteBuffer(m_hQueue, m_hTextures, CL_TRUE, 0,
                             gTextureDepth * gTextureWidth * gTextureHeight,
                             m_textures, 0, NULL, NULL));
  ...
  if (video)
    CHECKSTATUS(clEnqueueWriteBuffer(
        m_hQueue, m_hVideo, CL_TRUE, 0,
        gKinectColorVideo * gVideoWidth * gVideoHeight, video, 0, NULL, NULL));
  if (depth)
    CHECKSTATUS(clEnqueueWriteBuffer(
        m_hQueue, m_hDepth, CL_TRUE, 0,
        gKinectColorDepth * gDepthWidth * gDepthHeight, depth, 0, NULL, NULL));

  // Setting kernel arguments
  CHECKSTATUS(clSetKernelArg(m_hMainKernel, 0, sizeof(cl_int), (void *)&width));
  CHECKSTATUS(
      clSetKernelArg(m_hMainKernel, 1, sizeof(cl_int), (void *)&height));
  CHECKSTATUS(
      clSetKernelArg(m_hMainKernel, 2, sizeof(cl_mem), (void *)&m_hBitmap));
  CHECKSTATUS(
      clSetKernelArg(m_hMainKernel, 3, sizeof(cl_mem), (void *)&m_hBuffer));
  CHECKSTATUS(
      clSetKernelArg(m_hMainKernel, 4, sizeof(cl_mem), (void *)&m_hVideo));
  CHECKSTATUS(
      clSetKernelArg(m_hMainKernel, 5, sizeof(cl_mem), (void *)&m_hDepth));
  CHECKSTATUS(
      clSetKernelArg(m_hMainKernel, 6, sizeof(cl_mem), (void *)&m_hTextures));
  CHECKSTATUS(
      clSetKernelArg(m_hMainKernel, 7, sizeof(cl_int), (void *)&m_offset));
  CHECKSTATUS(
      clSetKernelArg(m_hMainKernel, 8, sizeof(cl_float), (void *)&value));
  CHECKSTATUS(
      clSetKernelArg(m_hMainKernel, 9, sizeof(cl_float), (void *)&m_timer));

  size_t globalWorkSize[] = {width, height};
  // run initial kernel
  CHECKSTATUS(clEnqueueNDRangeKernel(m_hQueue, m_hMainKernel, 2, NULL,
                                     globalWorkSize, 0, 0, 0, 0));
  ...
  // Bitmap
  if (bitmap != 0) {
    CHECKSTATUS(clEnqueueReadBuffer(m_hQueue, m_hBitmap, CL_FALSE, 0,
                                    width * height * sizeof(BYTE) * gColorDepth,
                                    bitmap, 0, NULL, NULL));
  }
...
void OpenCLKernel::setTexture(int index, BYTE *texture) {
  BYTE *idx =
      m_textures + index * gTextureWidth * gTextureHeight * gTextureDepth;
  int j(0);
  for (int i(0); i < gTextureWidth * gTextureHeight * gColorDepth;
       i += gColorDepth) {
    idx[j] = texture[i + 2];
    idx[j + 1] = texture[i + 1];
    idx[j + 2] = texture[i];
...
#ifdef USE_KINECT
long OpenCLKernel::updateSkeletons(double center_x, double center_y,
                                   double center_z, double size, double radius,
                                   int materialId, double head_radius,
                                   int head_materialId, double hands_radius,
                                   int hands_materialId, double feet_radius,
                                   int feet_materialId) {
  bool found = false;
      ...
      if (m_skeletonsBody == -1) {
        ...
        for (int j = 0; j < 20; j++) {
          ...
          case NUI_SKELETON_POSITION_HAND_RIGHT:
            ...
            {
               m_viewDir.s[0] =
            size*m_skeletonFrame.SkeletonData[i].SkeletonPositions[j].x;
               m_viewDir.s[1] =
            size*m_skeletonFrame.SkeletonData[i].SkeletonPositions[j].y;
               m_viewDir.s[2] = m_viewPos.s[2] +
            size*m_skeletonFrame.SkeletonData[i].SkeletonPositions[j].z;
            }
            */
            break;
          case NUI_SKELETON_POSITION_HEAD:
            ...
            size*m_skeletonFrame.SkeletonData[i].SkeletonPositions[j].y;
            m_viewPos.s[2] =
            size*m_skeletonFrame.SkeletonData[i].SkeletonPositions[j].z - 400.f;
            m_viewDir = m_viewPos;
            */
            break;
          }
          if (createSphere)
            setPrimitive(
                ...
                static_cast<float>(
                    ...
                    center_y),
                static_cast<float>(
                    m_skeletonFrame.SkeletonData[i].SkeletonPositions[j].z *
                        size +
                    center_z),
                static_cast<float>(r), static_cast<float>(r), m, 1);
        }
#if 0
