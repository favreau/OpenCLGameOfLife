...
OpenCLKernel::OpenCLKernel( int platformId, int deviceId, int nbWorkingItems, int draft )
 : m_hContext(0),m_hQueue(0), m_hBitmap(0), m_hBuffer(0),
...
#endif // USE_KINECT
   m_hTextures(0), m_textures(0), m_texturedTransfered(false),
   m_offset(-1), m_timer(0.f)
{
   int  status(0);
   ...
   const std::string& options)
{
   try 
   {
      ...
      //if( m_computeUnits == 0 ) 
      {
         clGetKernelWorkGroupInfo( m_hMainKernel, m_hDevices[0], CL_KERNEL_WORK_GROUP_SIZE, sizeof(m_computeUnits), &m_computeUnits , NULL);
         std::cout << "CL_KERNEL_WORK_GROUP_SIZE=" << m_computeUnits << std::endl;
      }

      clGetKernelWorkGroupInfo( m_hMainKernel, m_hDevices[0], CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE, sizeof(m_preferredWorkGroupSize), &m_preferredWorkGroupSize , NULL);
   ...
   int        height)
{
   ...
   m_hTextures = clCreateBuffer( m_hContext, CL_MEM_READ_ONLY , gTextureWidth*gTextureHeight*gTextureDepth*sizeof(BYTE), 0, NULL);
...
void OpenCLKernel::releaseDevice()
{
   ...
   if( m_hBuffer )     CHECKSTATUS(clReleaseMemObject(m_hBuffer));
   if( m_hVideo )      CHECKSTATUS(clReleaseMemObject(m_hVideo));
   if( m_hDepth )      CHECKSTATUS(clReleaseMemObject(m_hDepth));

   if( m_hMainKernel ) CHECKSTATUS(clReleaseKernel(m_hMainKernel));
   ...
   float value)
{
   ...
   {
      CHECKSTATUS(clEnqueueWriteBuffer( m_hQueue, m_hTextures,   CL_TRUE, 0, gTextureDepth*gTextureWidth*gTextureHeight,m_textures,   0, NULL, NULL));
